# 3_models

행동 인식 모델과 객체 탐지 모델을 학습하는 폴더입니다.  
이 폴더의 학습 코드는 **연산 자원 문제로 인해 로컬 환경이 아닌 Google Colab에서만 실행하는 것을 전제로 작성되었습니다.**

---

## 1. 역할

이 폴더는 전처리된 데이터를 이용해  
행동 인식 모델과 객체 탐지 모델을 학습하는 단계를 담당합니다.

주요 대상 모델:
- TCN 기반 행동 인식 모델
- YOLO 기반 객체 탐지 모델

학습은 GPU 자원이 필요한 작업이므로,  
본 프로젝트에서는 **Google Colab 환경에서만 학습을 수행**합니다.

---

## 2. 실행 환경

이 폴더의 코드는 **Google Colab 전용 학습 코드**입니다.

실행 환경:
- Google Colab
- Python 3.x
- Colab GPU 런타임
- Google Drive 연동

학습 과정에서 다음과 같은 Colab 기능을 사용할 수 있습니다.

- `google.colab.drive`를 이용한 Drive 마운트
- `/content/` 경로 기반 파일 처리
- Colab 런타임 GPU 사용
- 압축 해제 및 패키지 설치 셀 실행

즉, 이 폴더는 로컬 PC나 Raspberry Pi가 아니라  
**Colab에서 모델 학습을 수행하기 위한 영역**입니다.

---

## 3. 왜 Colab에서만 학습하는가

행동 인식 모델과 객체 탐지 모델 학습은
CPU 환경이나 저사양 로컬 환경에서 수행하기 어렵거나 비효율적일 수 있습니다.

본 프로젝트에서는 다음 이유로 학습을 Colab에서 수행합니다.

- GPU 자원 확보
- 학습 시간 단축
- 로컬 PC 자원 부담 감소
- 실험 및 모델 저장 관리 용이성

따라서 학습 단계는 개발 환경과 분리하여  
**Google Colab 기반으로 독립 실행**하도록 구성합니다.

---

## 4. 주요 기능

- 전처리 데이터 불러오기
- 모델 정의
- 학습 및 검증
- 모델 가중치 저장
- 성능 확인
- 실험 결과 기록

---

## 5. 입력 데이터

학습에 사용하는 입력 데이터는 주로 전처리 단계에서 생성됩니다.

예:
- 랜드마크 NPZ
- 시계열 텐서 데이터
- 이미지 데이터
- YOLO 형식 라벨 데이터
- train / val / test split 정보

일반적으로 이 데이터는 로컬 또는 프로젝트 폴더에서 정리한 뒤  
Google Drive로 옮겨 Colab에서 불러와 사용합니다.

---

## 6. 출력 데이터

학습 결과로 다음과 같은 파일이 생성될 수 있습니다.

- 학습된 모델 가중치 파일
- 체크포인트
- 학습 로그
- 검증 결과
- 추론용 best model 파일

이 출력물 역시 보통 Google Drive에 저장한 뒤  
이후 예측 단계(`4_predict`)에서 사용합니다.

---

## 7. 기본 학습 흐름

### 1) 전처리 결과 준비
`2_preprocessing`에서 생성한 학습 데이터를 준비합니다.

### 2) Google Drive 업로드
학습에 필요한 데이터와 코드 파일을 Google Drive에 업로드합니다.

### 3) Colab 런타임 설정
Google Colab에서 GPU 런타임을 활성화합니다.

### 4) Drive 마운트
학습 데이터와 저장 경로를 사용하기 위해 Drive를 마운트합니다.

예시:
```python
from google.colab import drive
drive.mount('/content/drive')